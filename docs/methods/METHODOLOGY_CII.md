`docs\methods\METHODOLOGY_CII.md`
<!-- repo path: docs/methods/protocol__a_pragmatics_of_engagement.md -->
<p align="center">
  <img src="../assets/midnight-darkmagic/repo_header_midnight.svg" alt="CII: A Pragmatics of Engagement — header" width="100%">
</p>
...
This document supplies the **governance grammar** for CII operations. It depends on abstractions (§1 principles); the downstream protocols implement the detail. This is Dependency Inversion at the methodology level.

```
METHODOLOGY_CII.md (policy / abstractions)
    ↑ depends on ↑
...
## §6. SOLID Self-Diagnostic (CII Violation Grammar)

SOLID principles, transposed into threshold mechanics (see [SOLID.md](../lib/SOLID.md) and [LENS__solid_threshold_mechanics.md](../lenses/LENS__solid_threshold_mechanics.md)), provide a self-diagnostic grammar for CII operations. Each SOLID principle maps to a specific CII failure mode.

### 6.1 CII Violation Taxonomy

| Rule | Name | CII Symptom | CARDS | Example |
|---|---|---|---|---|
...
play loadContext(task, tier):
  ...
  context := [METHODOLOGY_CII.md(dispatch_table)]
...
play sessionClose(task):
  ...
  task.primary_artifact.append("## Resume Instructions",
    ...
    "3. Read METHODOLOGY_CII.md §0 dispatch table for tier routing.",
...
2. **The governance paradox.** A methodology that governs CII must itself be maintained by CI. This creates a self-referential loop. Mitigation: the methodology is versioned (v4.2.0), and changes follow T2+ protocol with explicit sign-off.
