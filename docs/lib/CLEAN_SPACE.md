...
**Clean Space is a spatial facilitation methodology developed by David Grove (1950–2008) in the early 2000s.**
...
- Lawley, J. & Tompkins, P. 2011. "Symbolic Modelling: Emergent Change through Metaphor and Clean Language." Chapter in edited academic volume.
...
**Lineage:**
```
Clean Language (Grove, 1980s)
    └── Symbolic Modelling (Lawley & Tompkins, 2000)
            ├── Clean Space (Grove, 2000s)
            │       └── Emergent Knowledge (Grove, 2000s)
            └── Systemic Modelling (Walker, 2014)
                    └── Clean for Teams (Walker)
```

**Sibling entry:** [CLEAN_LANGUAGE.md](CLEAN_LANGUAGE.md) covers the verbal inquiry methodology. This entry covers the spatial methodology and the broader Symbolic Modelling process.
...
**Physical space becomes psychoactive when populated with a person's symbolic content.**

When a client stands in a particular spot and places a note representing an aspect of their thinking, that spot acquires meaning. Moving to a different spot changes what is accessible. Looking back at the first spot from the new position reveals something different from what was visible while standing in it.
...
| # | Question | Function |
|---|----------|----------|
| 1 | "And what do you know... from here?" | Elicits what the client knows from current position |
| 2 | "And is there anything else you know... from here?" | Expands the knowledge available at this position |
| 3 | "And what do you know... from here... about [space X]?" | Explores relationship between current space and another |
| 4 | "And what kind of space is this space?" | Characterises the space itself |
| 5 | "And is there a space that knows something about [X]?" | Invites discovery of a new space |
| 6 | "And what do you know now?" | Integration question — what has emerged from the whole process |

#### The 4 Directing Statements

| # | Statement | Function |
|---|-----------|----------|
| 1 | "Find a space that knows something about [X]" | Initiates movement to a new space |
| 2 | "Write that down" (or "Put that there") | Materialises knowledge onto paper/Post-It |
| 3 | "Go to [space X]" | Directs return to a previously established space |
| 4 | "Find a space that knows something else" | Prompts exploration beyond current network |
...
```
1. ENTRY
   Client writes or draws their topic/desired outcome on paper
   → Places it somewhere in the room (this becomes Space B)

2. FIRST SPACE
   "Find a space that knows something about [topic]"
   → Client moves to a position; stands there
   "And what do you know... from here?"
   → Client reports; facilitator may ask "anything else?"

3. POPULATE
   "And is there another space that knows something?"
   → Client finds new positions; each becomes a node
   → At each: "What do you know from here?"
   → Post-It notes materialise the knowledge at each location

4. NETWORK
   "And what do you know from here about [other space]?"
   → Client looks at other spaces from current position
   → Relationships between spaces emerge
   → Critical mass builds

5. EMERGENCE
   Once sufficient spaces exist and relationships connect them:
   → Emergent properties appear — patterns, insights, structures
      that could not be predicted from individual spaces
   → The client's system self-organises

6. INTEGRATION
   "And what do you know now?"
   → Client surveys the whole spatial network
   → New understanding is articulated
```
...
| Set-Up | Involves | Purpose |
|--------|----------|---------|
| **A** | The subject (person) | The one whose knowledge is being externalised |
| **B** | The object (written topic/goal) | The anchor — what the session is about |
| **C** | The space between A and B | Where relationship and tension live |
| **D** | Spaces outside the A-B-C boundary | Where new perspectives come from |
| **f** | The facilitator | Holds process; stays clean |
| **Network** | All spaces and their relationships | Where emergence happens |
...
1. **Sufficient diversity** — enough different spaces exist (typically 5–15+)
2. **Sufficient connectivity** — the spaces have been viewed from each other's perspectives
3. **Sufficient iteration** — the client has moved between spaces enough times for patterns to stabilise

At this threshold, the network produces **emergent properties** — features and functions that exist in the relationships between spaces, not in any individual space. This is where the insight lives.
...
1. **Metaphor** — working with the client's naturally arising metaphors
2. **Modelling** — the discipline of identifying and mapping patterns
3. **Clean questions** — the specific tool used for modelling

The Symbolic Modelling process follows five stages:

| Stage | Name | What Happens |
|-------|------|--------------|
| 1 | **Entry** | Surface the presenting condition and desired outcome |
| 2 | **Developing** | Build the metaphor landscape (or spatial network) |
| 3 | **Exploring** | Identify relationships, binds, resources, patterns |
...
**Key distinction:** Clean Language is the *tool*. Symbolic Modelling is the *process* that uses the tool. Clean Space is a *spatial variant* of the tool. All three belong to the same family and share the same principles (minimal projection, client sovereignty, emergence over prescription).
...
```
Internal model (rich, interconnected, multi-dimensional)
        ↓ translation gap ↓
External presentation (linear, simplified, audience-adapted)
```

Conventional advice: "Draw it on a whiteboard." "Sketch the architecture." "Make it visual."
...
| What it requires | What it does not require |
|------------------|--------------------------|
| Ability to walk to a spot in a room | Ability to draw or diagram |
| Ability to say what you know "from here" | Ability to visualise abstract relationships |
| Willingness to move and stand | Artistic or spatial-visual skill |
| Trust in the process | Prior knowledge of facilitation frameworks |

The externalisation happens through **kinesthetic and proprioceptive channels**:
- Standing in a position and reporting what you know
- Moving to another position and noticing what changes
- Looking at one space from another and discovering relationships
...
|-------|---------------------------|----------------------------|
| **1st order** | Helps prepare a presentation (content, slides, story, delivery) | Guided through someone else's structure |
| **2nd order** | Provides a process for discovering your own structure | Discovers their own organisation of the material |

Clean Space is a 2nd-order tool because:
- The facilitator does not need domain expertise (they ask "what do you know from here?" — not "explain your architecture")
- The structure emerges from the client's own spatial logic
- The client retains full ownership of the result
- The facilitator facilitates the facilitation, not the content
...
1. **A spatial map** — Post-It notes arranged in the room showing the components of their thinking and how they relate
2. **A natural ordering** — the sequence in which they visited spaces often reveals a narrative arc they weren't conscious of
3. **Key relationships** — connections between components that were implicit become explicit ("Oh — *that's* why the authentication layer matters to the user experience")
4. **An entry point** — whichever space they found first or returned to most often is frequently the natural opening for their presentation
5. **Confidence** — having walked their own thinking, they know they own it. It is not someone else's template filled in with their content.
...
|------|------------------------------|
| **C** (Competence) | Client determines all content. Facilitator asks "what do you know?" — presupposing the client *does* know. |
| **A** (Autonomy) | Client chooses where to stand, when to move, how many spaces to create, when to stop. No facilitator direction of content or structure. |
| **R** (Relatedness) | Facilitator is genuinely present — attentive to the client's movement, words, and pace. Holds the space (literally and figuratively). |
...
If CARDS degradation detected during Clean Space facilitation:
  C low → Am I telling the client what their space "means"?
  A low → Am I suggesting where they should stand next?
  R low → Am I watching from outside rather than being attentively present?
  D low → Am I questioning or reinterpreting their spatial arrangement?
  S low → Am I pushing them to move faster or create more spaces than feels right?
...
|-----------|---------------|-------------|
| Primary medium | Verbal metaphor | Physical space and movement |
| Toolkit size | ~12 questions | 6 questions + 4 directing statements |
| What gets externalised | Internal symbolic landscape (described) | Spatial network (walked, stood in, seen from) |
| Session modality | Seated dialogue | Client moves around room |
| Emergence mechanism | Iterative metaphor development | Network effects across populated spaces |
...
```
Group Clean Space:
  Each participant finds their own space for a shared topic
  → Multiple spatial perspectives coexist in the same room
  → No perspective is privileged; all are developed
  → The group's network of spaces reveals collective structure

This IS multi-partial hosting — expressed through spatial arrangement
...
- A space has **knowledge that is accessible from within it** (sensual qualities)
- A space has **knowledge that only becomes visible from other spaces** (withdrawn qualities)
- No single space contains the full picture — the system is **irreducible to its parts**
...
| SOC Component | Clean Space Mechanism |
|--------------|----------------------|
| **Comprehensibility** | Spaces make thinking visible and arranged. The client can *see* their own cognitive structure laid out in the room. |
...
- **Wisdom of the system** — trusting that the client's network of spaces contains more than the sum of its parts
- **Questions-based inquiry** — the process is driven by questions, not answers
...
**Use when:** Helping someone externalise and organise their thinking on a specific topic.

**Materials:** Post-It notes or A4 paper; pens; a room with space to move.

```
1. FRAME
   "What would you like to have happen?"
   → Client states their topic or desired outcome
   → Client writes it on paper and places it somewhere in the room

2. FIRST SPACE
   "Find a space that knows something about [topic]."
   → Client walks to a spot
   "And what do you know... from here?"
   → Client speaks; facilitator listens
   "Write that down."
   → Client writes key words on a Post-It; places it on the floor

3. POPULATE (repeat 5–15 times)
   "And is there another space that knows something?"
   → Client moves to new spot
   "And what do you know... from here?"
   "And anything else from here?"
   "Write that down."
   → Each space gets its Post-It

4. NETWORK
   At each new space, also ask:
   "And what do you know from here about [earlier space]?"
   → Client looks at earlier spaces from current position
   → Relationships and tensions surface

5. EMERGENCE
   Watch for: shifts in voice, energy, posture; moments of surprise;
   statements like "Oh!" or "I didn't realise..." or "That's why..."
   → Do not interpret. Do not celebrate. Stay clean.
   "And what do you know now?"

6. CLOSE
   "And from everything you know now... what do you know?"
   → Client surveys the whole network
   → Integration happens
```

**Trap to avoid:** Suggesting where the client should stand. If they hesitate, wait. The process belongs to them.

### Routine B: Clean Space for Presentation Coaching (SkillsClinic Frame)

**Use when:** Helping a technical professional discover their presentation structure.

```
1. FRAME
   "What's the thing you need to present?"
   → Client writes topic on paper; places it in the room
   "And what would you like the audience to know/feel/do?"
   → Client writes this too; places it (this anchors the outcome)

2. POPULATE
   "Find a space that knows something about [topic]."
   → Build the spatial network as in Routine A
   → Each space represents an aspect of what the client knows

3. AUDIENCE SPACE
   "And is there a space that knows something about the audience?"
   → Client finds a position that represents audience perspective
   "And what does the audience know from here?"
   → Looking at the client's network FROM the audience's position
      reveals what is clear and what is missing

4. SEQUENCE
   "And if you were to walk through these spaces in order...
    where would you start?"
   → Client walks a path through their spaces
   → This path IS the presentation structure
   → Note the order — it is the client's natural narrative arc

5. GAPS
   "And is there a space that knows something that hasn't
    been spoken yet?"
   → Surfaces what the client knows but hadn't externalised
   → These are often the crucial transitions or assumptions

6. CLOSE
   "And from everything you know now, what's the first thing
    you'd say to that audience?"
   → The client now has: a spatial map, a sequence, an opening,
      and confidence that the structure is their own
```

**Trap to avoid:** Turning it into slide preparation. The output of this routine is a *structure*, not a deck. The slides come later, built on the spatial logic the client has discovered.

### Routine C: Group Clean Space

**Use when:** A team needs to surface collective knowledge on a shared challenge.

```
1. SHARED TOPIC
   The team agrees on a question or challenge
   → Written and placed centrally

2. INDIVIDUAL SPACES
   Each team member simultaneously finds their own space
   "Find a space that knows something about [topic]."
   "And what do you know from here?"
   → Each person writes and places their knowledge

3. VISITING
   Team members visit each other's spaces
   "Go stand in [Person B]'s space."
   "And what do you know from here about [your own space]?"
   → Cross-pollination of perspectives

4. COLLECTIVE EMERGENCE
   "Is there a space that no one has found yet?"
   → The group discovers spaces that belong to the system,
      not to any individual

5. INTEGRATION
   The group surveys the full spatial network together
   "And what does the team know now?"
```
...
- **[CLEAN_LANGUAGE.md](CLEAN_LANGUAGE.md)** — Sibling entry; verbal inquiry methodology; Clean Space extends CL into spatial domain
- **[Library Index](README.md)** — Registry of imported constructs
- **[CARDS.md](CARDS.md)** — Human needs framework; Clean Space as CARDS-preserving spatial facilitation
- **[MULTI_PARTIALITY.md](MULTI_PARTIALITY.md)** — Multi-partial stance; group Clean Space as spatial multi-partiality
...
<!--
Easter Egg: And what do you know... from here?

You're standing in the source code. This is a space too.
And what does this space know that the rendered page does not?
