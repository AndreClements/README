...
**Hyperstrate** :: The stack of stuff that co-authors you (tools, UIs, policies). **|** The superposed, maculate milieu of non-negotiable, often opaque, co-authorship.
...
**Mapping (𝒫→𝒜)** :: The process, not the product; the thing we actually read. **|** The semiotic ground where meaning stabilises as invariants under perturbation.

**Invariant** :: A feature that stays stable when you nudge things. **|** A feature *f*∈ℱ that remains within an ε-ball under small parameter changes ΔP.

**Designed Transition** :: A change you planned for, triggered by a specific shock. **|** An intended feature change that tracks predictably with a declared perturbation Δ★.
...
**Defaults Digest** :: A fingerprint of the tool's hidden assumptions. **|** A cryptographic hash of a host’s default parameters, used to detect and audit drift.
...
**The ε-gap / theSmallDiffGap** :: The smallest difference that actually matters. **|** The liminal joint between dual-coding and non-dual axiom; the minimal condition for agency to exist.

**Let Sobriety** :: The practice of avoiding naive materialism and performative spiritualism. **|** An epistemic stance that holds two truths: operating via dualistic contracts while grounded in a non-dual axiom.
