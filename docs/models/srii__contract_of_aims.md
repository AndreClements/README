...
*   SRII Invariants **SHOULD** include **Continuity Integrity** (small changes do not produce unmarked meaning-cliffs).
...
1.  **Identity Integrity:** Entities and Relations MUST have stable identity across transformations (rename, refactor, rehost). *No silent aliasing.*
...
3.  **Provenance Integrity:** Every meaningful state change MUST be attributable to an origin chain (who/what/when/which rule). "Unknown" is a valid value; "Null/Hidden" is not.
...
6.  **Irreversibility Marking:** If an action is irreversible (or costly to reverse), the system MUST represent that as a structural property (e.g., via `d_time` distance).
